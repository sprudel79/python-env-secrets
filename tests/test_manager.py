...
def manager(
    ...
    return UserSecretsManager(project_dir=project_dir, auto_init=True)
...
class TestInit:
    def test_creates_env_file_with_guid(self, manager: UserSecretsManager) -> None:
        assert manager.env_file_path.exists()
        content = manager.env_file_path.read_text()
        assert "USER_SECRETS_ID=" in content
        assert manager.user_secrets_id is not None

    def test_creates_secrets_file(self, manager: UserSecretsManager) -> None:
    ...
    def test_reuses_existing_guid(
        ...
        project_dir.mkdir(parents=True, exist_ok=True)
        m1 = UserSecretsManager(project_dir=project_dir, auto_init=True)
        guid1 = m1.user_secrets_id

        m2 = UserSecretsManager(project_dir=project_dir, auto_init=True)
        assert m2.user_secrets_id == guid1
    ...
    def test_preserves_existing_env_content(
        ...
        UserSecretsManager(project_dir=project_dir, auto_init=True)
...
class TestSetGetDeleteClear:
    def test_set_and_get(self, manager: UserSecretsManager) -> None:
        manager.set("DB_URL", "postgres://localhost/test")
        assert manager.get("DB_URL") == "postgres://localhost/test"

    def test_set_updates_environ(self, manager: UserSecretsManager) -> None:
        manager.set("MY_KEY", "my_value")
        assert os.environ.get("MY_KEY") == "my_value"
        # cleanup
        os.environ.pop("MY_KEY", None)

    def test_get_nonexistent_returns_none(self, manager: UserSecretsManager) -> None:
        assert manager.get("NO_SUCH_KEY") is None

    def test_delete_existing(self, manager: UserSecretsManager) -> None:
        manager.set("TO_DELETE", "val")
        assert manager.delete("TO_DELETE") is True
        assert manager.get("TO_DELETE") is None

    def test_delete_nonexistent(self, manager: UserSecretsManager) -> None:
        assert manager.delete("NOPE") is False

    def test_clear(self, manager: UserSecretsManager) -> None:
        manager.set("A", "1")
        manager.set("B", "2")
        count = manager.clear()
        assert count == 2
    ...
    def test_list(self, manager: UserSecretsManager) -> None:
        manager.set("X", "10")
        manager.set("Y", "20")
        result = manager.list()
...
class TestLoad:
    def test_load_injects_env(self, manager: UserSecretsManager) -> None:
...
class TestEdgeCases:
    def test_value_with_spaces_is_quoted(self, manager: UserSecretsManager) -> None:
        manager.set("SPACED", "hello world")
        assert manager.get("SPACED") == "hello world"

    def test_value_with_equals(self, manager: UserSecretsManager) -> None:
        manager.set("CONN", "host=localhost;port=5432")
        assert manager.get("CONN") == "host=localhost;port=5432"

    def test_overwrite_existing_secret(self, manager: UserSecretsManager) -> None:
        manager.set("KEY", "v1")
        manager.set("KEY", "v2")
        assert manager.get("KEY") == "v2"

    def test_info_returns_dict(self, manager: UserSecretsManager) -> None:
        info = manager.info()
        assert "user_secrets_id" in info
...
class TestUninitialised:
    def test_raises_when_not_initialised(
        ...
        project_dir.mkdir(parents=True, exist_ok=True)
        m = UserSecretsManager(project_dir=project_dir, auto_init=False)
